        ...
        <div className="relative">
          <div className="flex flex-row gap-6 overflow-x-auto scrollbar-hide pb-2 justify-start items-stretch snap-x snap-mandatory pr-8" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
            {destinations.map((destination, idx) => (
              <Card key={destination.name} className="flex flex-row items-stretch w-[400px] min-w-[380px] max-w-[420px] bg-white rounded-2xl shadow-lg p-0 overflow-hidden border-0 flex-shrink-0 snap-center">
                {/* Image */}
                ...
                <div className="flex flex-col justify-between flex-1 px-3 py-3">
                  <div>
                    <h3 className="text-xl font-bold text-[#023e8a] mb-1 leading-tight">{destination.name}</h3>
                    <div className="text-sm text-[#333] font-medium mb-1 truncate max-w-[120px]">{destination.description}...</div>
                    <div className="flex items-center mb-2">
                      {[...Array(5)].map((_, i) => (
                        <svg key={i} className={`w-4 h-4 ${i < Math.round(destination.rating) ? 'text-[#ffc107]' : 'text-gray-300'}`} fill="currentColor" viewBox="0 0 20 20">
                          <polygon points="9.9,1.1 7.6,6.6 1.6,7.6 6,11.7 4.9,17.6 9.9,14.7 14.9,17.6 13.8,11.7 18.2,7.6 12.2,6.6" />
                        </svg>
                      ))}
                      <span className="ml-2 text-xs text-[#333] font-semibold">({destination.reviews})</span>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-row gap-2 mt-2 items-center">
                    <div className="bg-[#f1f6fb] text-[#023e8a] text-base font-semibold rounded-xl px-4 py-2 text-center min-w-[90px]">
                      {destination.properties.replace('properties', 'Properties').replace('k', 'k+')}
                    </div>
