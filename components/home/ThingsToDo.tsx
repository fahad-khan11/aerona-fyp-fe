...
export function ThingsToDo() {
  const [activityImages, setActivityImages] = useState<Record<string, UnsplashImageResult | null>>({})
    ...
    const fetchImages = async () => {
      try {
        ...
        const imagePromises = activities.map(async (activity) => {
          try {
            const results = await searchPhotos(activity.imageQuery, 1, 1);
            ...
            return { name: activity.name, image: mainImage };
          } catch (error) {
  ...
  const getActivityImage = (activityName: string): string => {
    const unsplashImage = activityImages[activityName];
    const fallbackImage = activities.find(a => a.name === activityName)?.image || '';
    ...
    <div className="container py-16">
      <div className="flex items-center justify-between mb-8">
        <h2 className="text-4xl font-bold text-[#333333] relative">
          Top <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#023e8a] to-[#00b4d8]">Things to Do</span>
          ...
          >
            ...
            <div className="p-5">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-xs text-[#00b4d8]">{activity.location}</span>
              </div>
              <h3 className="font-bold text-lg text-[#333333] mb-1">{activity.name}</h3>
                ...
                <div className="flex items-center gap-2">
                  <div className="flex gap-1">
                    {[...Array(5)].map((_, i) => (
                      <div 
                        key={i}
                        className={`w-4 h-4 rounded-full ${i < Math.floor(activity.rating) ? 'bg-[#00b4d8]' : 'bg-white/30'}`}
                      />
                    ))}
                  </div>
                  <span className="text-[#00b4d8] text-sm ml-2">{activity.rating}</span>
                  <span className="text-gray-500 text-xs ml-2">({activity.reviews})</span>
                </div>
              </div>
              <div className="border-t border-gray-100 mt-3 pt-3">
                <span className="text-lg font-bold text-[#023e8a]">{activity.price}</span>
                {activity.price !== "Free" && (
